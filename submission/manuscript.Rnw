...
To put the power of the Fisher test into perspective, we can compare its power to reject the null based on one statistically nonsignificant result ($k=1$) with the power of a regular $t$-test to reject the null. If $\eta=.1$, the power of a regular $t$-test equals \Sexpr{tmp_power(.1, 33, 3)}, \Sexpr{tmp_power(.1, 62, 3)}, \Sexpr{tmp_power(.1, 119, 3)} for sample sizes of 33, 62, 119, respectively; if $\eta$ = .25, power values equal \Sexpr{tmp_power(.25, 33, 3)}, \Sexpr{tmp_power(.25, 62, 3)}, \Sexpr{tmp_power(.25, 119, 3)} for these sample sizes. The power values of the regular $t$-test are higher than that of the Fisher test, because the Fisher test does not make use of the more informative statistically significant findings.
...
gend <- gend[!is.na(gend$final_code),]
sum(gend$Computed > .05 & gend$significance == 'significant')
gend <- gend[!(gend$Computed > .05 & gend$significance == 'significant'),]
...
if(!file.exists(sprintf('%sfigures/Fig6.pdf', run)))
{
    ...
    geom_density(alpha = .3, trim = TRUE) + 
    xlab(TeX("$P$-value")) + 
    ylab("Density")
