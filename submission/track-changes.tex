...
%DIF DEL review round 1/original.tex   Mon Jan 16 13:01:20 2017
%DIF ADD manuscript.tex                Wed Jan 18 11:50:10 2017
...
\begin{abstract}
Due to its probabilistic nature, Null Hypothesis Significance Testing (NHST) is subject to decision errors. The concern for false positives has overshadowed the concern for false negatives in the recent debates in psychology\DIFdelbegin \DIFdel{, which seems unwarranted. Our study demonstrates the importance of paying attention to false negatives alongside those significant results that may be 'too good to be true'; nonsignificant results may }\DIFdelend \DIFaddbegin \DIFadd{. This might be unwarranted,  since  reported statistically nonsignificant findings may just }\DIFaddend be 'too good to be false'. We examined evidence for false negatives in nonsignificant results in three different ways. \DIFdelbegin \DIFdel{To this end, we }\DIFdelend \DIFaddbegin \DIFadd{We }\DIFaddend adapted the Fisher method to detect the presence of at least one false negative in a set of statistically nonsignificant results\DIFdelbegin \DIFdel{; simulations indicated it to be }\DIFdelend \DIFaddbegin \DIFadd{. Simulations show that the adapted Fisher method generally is }\DIFaddend a powerful method \DIFdelbegin \DIFdel{for that purpose. The three applications indicated }\DIFdelend \DIFaddbegin \DIFadd{to detect false negatives. We examined evidence for false negatives in the psychology literature in three applications of the adapted Fisher method. These applications indicate }\DIFaddend that (i) \DIFaddbegin \DIFadd{the observed effect size distribution of nonsignificant effects exceeds the expected distribution assuming a null-effect, and }\DIFaddend approximately two out of three (66.7\%) psychology articles reporting nonsignificant results contain evidence for at least one false negative, (ii) nonsignificant results on gender effects contain evidence of true nonzero effects, and (iii) the statistically nonsignificant replications from the Reproducibility Project Psychology (RPP) do not warrant \DIFaddbegin \DIFadd{strong }\DIFaddend conclusions about the absence or presence of true zero effects underlying these nonsignificant results. \DIFdelbegin \DIFdel{False }\DIFdelend \DIFaddbegin \DIFadd{We conclude that false }\DIFaddend negatives deserve more attention in the \DIFdelbegin \DIFdel{debate among psychologists, because there is sufficient evidence in these three applications to indicate that false negatives are a threat to the scientific discovery process; it can be considered }\DIFdelend \DIFaddbegin \DIFadd{current debate on statistical practices in psychology. Potentially neglecting effects due to }\DIFaddend a \DIFaddbegin \DIFadd{lack of statistical power can lead to a }\DIFaddend waste of \DIFdelbegin \DIFdel{resources if effects are studied but results are neglected due to the lack of statistical power}\DIFdelend \DIFaddbegin \DIFadd{research resources and stifle the scientific discovery process}\DIFaddend .
\end{abstract}

\DIFdelbegin \DIFdel{\keywords{nonsignificant, underpowered, effect size, Fisher test}
}\DIFdelend \DIFaddbegin \DIFadd{\keywords{NHST, reproducibility project, nonsignificant, power, underpowered, effect size, Fisher test, gender}
}\DIFaddend 
...
Because effect sizes and their distribution typically overestimate population effect size \DIFaddbegin \DIFadd{$\eta^2$}\DIFaddend , particularly when sample size is small \cite{Voelkle2007-at,Hedges1981-og}, we also compared the observed and expected adjusted nonsignificant effect sizes that correct for such overestimation of effect sizes (right panel of Figure \ref{fig:fig3}; see Appendix B). \DIFaddbegin \DIFadd{Such overestimation affects all effects in a model, both focal and non-focal. }\DIFaddend The distribution of adjusted effect sizes of nonsignificant results tells the same story \DIFaddbegin \DIFadd{as the unadjusted effect sizes}\DIFaddend ; observed effect sizes are larger than expected effect sizes. For instance, the distribution of adjusted reported effect size suggests 49\% of effect sizes are at least small, whereas under the $H_0$ only 22\% is expected.

\subsubsection*{Evidence of false negatives in articles.}


The Fisher test was applied to the nonsignificant test results of each of the 14,765 papers separately, to inspect for evidence of false negatives. More technically, we inspected whether $p$-values within a paper deviate from what can be expected under the $H_0$ (i.e., uniformity). If $H_0$ is in fact true, our results would be that there is evidence for false negatives in 10\% of the papers (a meta-false positive). Table \ref{tab:tab4} shows the number of papers with evidence for false negatives, specified per journal and per $k$ number of nonsignificant test results. The first row indicates the number of papers that report no nonsignificant results. \DIFaddbegin \DIFadd{When $k=1$, the Fisher test is simply another way of testing whether the result deviates from a null effect, conditional on the result being statistically nonsignificant. }\DIFaddend Overall results (last row) indicate that 47.1\% of all articles show evidence of false negatives (i.e. 6,951 articles). Of articles reporting at least one nonsignificant result, 66.7\% show evidence of false negatives, which is much more than the 10\% predicted by chance alone. \DIFaddbegin \DIFadd{Results did not substantially differ if nonsignificance is determined based on $\alpha=.10$ (the analyses can be rerun with any set of $p$-values larger than a certain value based on the code provided on OSF; }\url{https://osf.io/qpfnw}\DIFadd{).
}\DIFaddend 
...
\DIFaddbegin \DIFadd{Very recently four statistical papers have re-analyzed the RPP results to either estimate the frequency of studies testing true zero hypotheses or to estimate the individual effects examined in the original and replication study. All four papers account for the possibility of publication bias in the original study. Johnson, Payne, Wang, Asher, and Mandal \mbox{%DIFAUXCMD
\cite{doi:10.1080/01621459.2016.1240079} }%DIFAUXCMD
estimated a Bayesian statistical model including a distribution of effect sizes among studies for which the null-hypothesis is false. On the basis of their analyses they conclude that at least 90\% of psychology experiments tested negligible true effects. Johnson et al.'s model as well as our Fisher's test are not useful for estimation and testing of individual effects examined in original and replication study. Interpreting results of individual effects should take the precision of the estimate of both the original and replication into account \mbox{%DIFAUXCMD
\cite{Cumming2014-fi}}%DIFAUXCMD
. Etz and Vandekerckhove \mbox{%DIFAUXCMD
\cite{10.1371/journal.pone.0149794} }%DIFAUXCMD
reanalyzed the RPP at the level of individual effects, using Bayesian models incorporating publication bias. They concluded that 64\% of individual studies did not provide strong evidence for either the null or the alternative hypothesis in either the original of the replication study. This agrees with our own and Maxwell's \mbox{%DIFAUXCMD
\cite{Maxwell2015-yb} }%DIFAUXCMD
interpretation of the RPP findings. As opposed to Etz and Vandekerckhove \mbox{%DIFAUXCMD
\cite{10.1371/journal.pone.0149794}}%DIFAUXCMD
, Van Aert and Van Assen \mbox{%DIFAUXCMD
\cite{rpp-hybrid,rpp-bayes} }%DIFAUXCMD
use a statistically significant original and a replication study to evaluate the common true underlying effect size, adjusting for publication bias. From their Bayesian analysis \mbox{%DIFAUXCMD
\cite{rpp-bayes} }%DIFAUXCMD
...
Much attention has been paid to false positive results in recent years. Our study demonstrates the importance of \DIFdelbegin \DIFdel{focusing on false negatives as well}\DIFdelend \DIFaddbegin \DIFadd{paying attention to false negatives alongside false positives}\DIFaddend . We examined evidence for false negatives in nonsignificant results in three different ways. \DIFdelbegin \DIFdel{To this end}\DIFdelend \DIFaddbegin \DIFadd{Specifically}\DIFaddend , we adapted the Fisher method to detect the presence of at least one false negative in a set of statistically nonsignificant results. Simulations indicated the adapted Fisher test to be a powerful method for that purpose. The three applications indicated that (i) approximately two out of three psychology articles reporting nonsignificant results contain evidence for at least one false negative, (ii) nonsignificant results on gender effects contain evidence of true nonzero effects, and (iii) the statistically nonsignificant replications from the Reproducibility Project Psychology (RPP) do not warrant \DIFaddbegin \DIFadd{strong }\DIFaddend conclusions about the absence or presence of true zero effects underlying these nonsignificant results (RPP does yield less biased estimates of the effect; the original studies severely overestimated the effects of interest).
...
\begin{figure}[!ht]
\centering
\includegraphics[width=0.75\textwidth]{appendix_a}
